...
import serviceFaqs from '@/data/service-faqs.json';
import styles from './page.module.css';

// Category-unique AEO FAQ (answer-first, no fabricated facts), keyed by category slug. Generated by
// paa-tree; interpolated per page. Category-specific questions keep 4.2k×area pages from being thin.
const SERVICE_FAQS: Record<string, Array<{ q: string; a: string }>> = serviceFaqs;
...
export default async function ServiceAreaPage({
  ...
  const heading = s.heading
    ...
    .replace('{locality}', data.localityName)
    .replace('{city}', data.cityName);
  // Category-unique AEO questions first (highest snippet/AI-Overview value), then the generic three.
  const categoryFaqs = (SERVICE_FAQS[data.category.slug] ?? []).map((item) => ({
    q: item.q.replace(/\{locality\}/g, data.localityName).replace(/\{count\}/g, String(data.total)),
    a: item.a.replace(/\{locality\}/g, data.localityName).replace(/\{count\}/g, String(data.total)),
  }));
  const faq = [
    ...categoryFaqs,
    {
